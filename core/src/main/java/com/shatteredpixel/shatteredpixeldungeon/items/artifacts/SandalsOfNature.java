...
public class SandalsOfNature extends Artifact {
	...
	private static final HashMap<Class<? extends Plant.Seed>, Integer> seedChargeRequest = new HashMap<>();
	static {
		seedChargeRequest.put(Rotberry.Seed.class,     8);
		seedChargeRequest.put(Firebloom.Seed.class,    20);
		seedChargeRequest.put(Swiftthistle.Seed.class, 20);
		seedChargeRequest.put(Sungrass.Seed.class,     80);
		seedChargeRequest.put(Icecap.Seed.class,       20);
		seedChargeRequest.put(Stormvine.Seed.class,    20);
		seedChargeRequest.put(Sorrowmoss.Seed.class,   20);
		seedChargeRequest.put(Dreamfoil.Seed.class,    12);
		seedChargeRequest.put(Earthroot.Seed.class,    40);
		seedChargeRequest.put(Starflower.Seed.class,   40);
		seedChargeRequest.put(Fadeleaf.Seed.class,     12);
		seedChargeRequest.put(Blindweed.Seed.class,    12);
	}
	...
	public void execute( Hero hero, String action ) {
		...
		if (action.equals(AC_FEED)){
			...
			else if (charge < seedChargeRequest.get(curSeedEffect))    GLog.i( Messages.get(this, "low_charge") );
			else {
	...
	public String desc() {
		...
		if (curSeedEffect != null){
			desc += "\n\n" + Messages.get(this, "desc_ability",
					...
					seedChargeRequest.get(curSeedEffect));
		}
	...
	public void restoreFromBundle( Bundle bundle ) {
		...
		if (bundle.contains(CUR_SEED_EFFECT))
			curSeedEffect = bundle.getClass(CUR_SEED_EFFECT);
		else if (!seeds.isEmpty())
			curSeedEffect = seeds.get(seeds.size()-1);
		...
		public void onSelect(Integer cell) {
			if (cell != null){
				...
				if (!Dungeon.level.heroFOV[cell] || Dungeon.level.distance(curUser.pos, cell) > 3){
					...
					Plant plant = ((Plant.Seed) Objects.requireNonNull(Reflection.newInstance(curSeedEffect))).couch(cell, null);
