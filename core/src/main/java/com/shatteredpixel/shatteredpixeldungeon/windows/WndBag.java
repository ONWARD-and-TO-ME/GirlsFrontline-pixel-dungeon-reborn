...
public class WndBag extends WndTabbed {
	...
	public WndBag( Bag bag, ItemSelector selector ) {
		...
		int i = 1;
		for (Bag b : Dungeon.cur().hero.belongings.getBags()) {
			if (b != null) {
	...
	public static WndBag lastBag( ItemSelector selector ) {
		...
		if (lastBag != null && Dungeon.cur().hero.belongings.backpack.contains( lastBag )) {

			return new WndBag( lastBag, selector );

		} else {

			return new WndBag( Dungeon.cur().hero.belongings.backpack, selector );
	...
	public static WndBag getBag( ItemSelector selector ) {
		if (selector.preferredBag() == Belongings.Backpack.class){
			...
			Bag bag = Dungeon.cur().hero.belongings.getItem( selector.preferredBag() );
	...
	protected void placeItems( Bag container ) {
		...
		// Equipped items
		Belongings stuff = Dungeon.cur().hero.belongings;
		...
		if (container != Dungeon.cur().hero.belongings.backpack){
			placeItem(container);
	...
	protected void placeItem( final Item item ) {
		...
		InventorySlot slot = new InventorySlot( item ){
			...
			protected void onClick() {
				if (lastBag != item && !lastBag.contains(item) && !item.isEquipped(Dungeon.cur().hero)){
