...
public class EntranceRoom extends StandardRoom {
	...
	public void paint( Level level ) {
		...
		if (Dungeon.cur().depth == 1 && !Document.ADVENTURERS_GUIDE.isPageRead(Document.GUIDE_INTRO)){
			int pos;
			do {
		...
		if (Dungeon.cur().depth == 2 && !Document.ADVENTURERS_GUIDE.isPageFound(Document.GUIDE_SEARCHING)){
			int pos;
			do {
				//can't be on bottom row of tiles
				pos = level.pointToCell(new Point( Random.IntRange( left + 1, right - 1 ),
