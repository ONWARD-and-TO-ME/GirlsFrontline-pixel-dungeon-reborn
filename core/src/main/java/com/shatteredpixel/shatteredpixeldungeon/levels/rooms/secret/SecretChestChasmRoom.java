...
public class SecretChestChasmRoom extends SecretRoom {
	...
	public void paint(Level level) {
		...
		level.drop(Generator.random(), level.pointToCell(p)).setType(Heap.Type.LOCKED_CHEST).setRoom(Heap.Room.FOUR_GOLDEN);
		...
		Painter.set(level, p, Terrain.EMPTY_SP);
		if (chests > 0) {
			level.drop(new GoldenKey(Dungeon.cur().depth), level.pointToCell(p));
			chests--;
		}
