...
public class TerrainFeaturesTilemap extends DungeonTilemap {
	...
	protected int getTileVisual(int pos, int tile, boolean flat){
		...
		int stage = (Dungeon.cur().depth-1)/5;
	...
	public static Image getTrapVisual( Trap trap ){
		...
		RectF uv = instance.tileset.get((trap.active ? trap.color : Trap.BLACK) + (trap.shape * Column));
	...
	public static Image getPlantVisual( Plant plant ){
		...
		RectF uv = instance.tileset.get(plant.image + 7*Column);
		if (uv == null) return null;
