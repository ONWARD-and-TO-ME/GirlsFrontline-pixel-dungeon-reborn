...
public class Web extends Blob {
	...
	protected void evolve() {
		...
		for (int i = area.left; i < area.right; i++){
			for (int j = area.top; j < area.bottom; j++){
				...
				l.flammable[cell] = off[cell] > 0 || (Terrain.flags[l.map[cell]] & Terrain.FLAMABLE) != 0;
			}
		}
	}
	...
	public void seed(Level level, int cell, int amount) {
		...
		level.flammable[cell] = cur[cell] > 0 || (Terrain.flags[level.map[cell]] & Terrain.FLAMABLE) != 0;
	}
	...
	public void clear(int cell) {
		...
		l.flammable[cell] = cur[cell] > 0 || (Terrain.flags[l.map[cell]] & Terrain.FLAMABLE) != 0;
	}
